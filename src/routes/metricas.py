...
def obter_metricas(db: Session = Depends(get_db)):
    ...
    chamados_automaticos = db.query(func.count(Chamado.id))\
        .filter(Chamado.encaminhado_para_humano == False)\
        .scalar() or 0
    chamados_encaminhados = db.query(func.count(Chamado.id))\
        .filter(Chamado.encaminhado_para_humano == True)\
        .scalar() or 0
    
    total_clientes = db.query(func.count(Cliente.id)).scalar() or 0
    
    taxa_resolucao = (chamados_automaticos / total_chamados * 100) if total_chamados > 0 else 0
...
def metricas_por_canal(db: Session = Depends(get_db)):
    ...
    for canal in canais:
        total = db.query(func.count(Chamado.id))\
            .filter(Chamado.canal == canal)\
            .scalar() or 0
        automaticos = db.query(func.count(Chamado.id))\
...
def metricas_por_status(db: Session = Depends(get_db)):
    ...
    for status in statuses:
        count = db.query(func.count(Chamado.id))\
